...
output <- split_str_to_dataframe(output, abbreviation = "nr.")
...
# tjekker længste sætninger
output %>%
  mutate(antal_ord = stringr::str_count(output$sentences, "\\S+")) %>%
  arrange(desc(antal_ord)) %>%
  top_n(10) %>%
  select(data)
...
# gemmer data
ved_soen_clean <- output
usethis::use_data(ved_soen_clean, overwrite = TRUE)
